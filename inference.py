...
class ParticleFilter(InferenceModule):
        ...
        if noisyDistance is None:
           ...
           # wt.normalize()
            ...
            if wt.totalCount() == 0:
                ...
                self.particles = []
                for i in range(self.numParticles):
                    self.particles.append(util.sample(wt))
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            particles.append(util.sample(newDist))
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for particle in particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] != None:
                    wt *= emissionModels[i][util.manhattanDistance(particle[i], pacmanPosition)]
            wts[particle] += wt
        ...
        if wts.totalCount() == 0:
            ...
            self.particles = []
            for i in range(self.numParticles):
                self.particles.append(tuple(util.sample(wts)))
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                newDist = getPositionDistributionForGhost(
                    setGhostPositions(gameState, oldParticle), i, self.ghostAgents[i]
                )
                newParticle[i] = util.sample(newDist)
            "*** END YOUR CODE HERE ***"
            newOnes.append(tuple(newParticle))
        self.particles = newOnes
